...
let at = '6c1c4c98-11db-4d30-9c0a-8d2e3f8b82b5';
let fallbackAddress = '';
let socks5Config = '';
...
const DEFAULT_REMOTE_CONFIG_URL =
  'https://raw.githubusercontent.com/byJoey/test/refs/heads/main/tist.ini';

let remoteConfigUrl = DEFAULT_REMOTE_CONFIG_URL;

// 伪装域名（用于节点的 Host/SNI），为空则使用当前 Worker 域名
let hd = '';
...
function getConfigValue(key, defaultValue) {
  try {
    if (kvConfig && Object.prototype.hasOwnProperty.call(kvConfig, key)) {
      const value = kvConfig[key];
...
function boolFromNotNo(value) {
  if (typeof value === 'boolean') return value;
  if (typeof value === 'string') {
    return value.toLowerCase() !== 'no' && value !== '0' && value.toLowerCase() !== 'false';
  }
  return Boolean(value);
}
...
async function saveKVConfig() {
  if (kvStore) {
    try {
      ...
      console.error('Failed to save KV config:', e);
    }
  }
}
...
async function initKVStore(env) {
  if (env.C) {
    try {
      kvStore = env.C;
      const val = await env.C.get('c');
      if (val) {
        kvConfig = JSON.parse(val);
      }
    } catch (e) {
      console.error(e);
    }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (hasConfigValue(vlessControl)) {
        ev = boolFromYes(vlessControl);
      }

      const tjControl = getConfigValue('et', env.et);
      if (hasConfigValue(tjControl)) {
        et = boolFromYes(tjControl);
      }

      tp = getConfigValue('tp', env.tp) || '';

      const xhttpControl = getConfigValue('ex', env.ex);
      if (hasConfigValue(xhttpControl)) {
        ex = boolFromYes(xhttpControl);
      }

      scu = getConfigValue('scu', env.scu) || 'https://url.v1.mk/sub';

      remoteConfigUrl =
        getConfigValue('rcu', env.rcu || env.RCU) ||
        getConfigValue('remoteConfigUrl', env.remoteConfigUrl || env.REMOTE_CONFIG_URL) ||
        DEFAULT_REMOTE_CONFIG_URL;

      hd = getConfigValue('hd', env.hd || env.HD) || '';

      const preferredDomainsControl = getConfigValue('epd', env.epd || 'no');
      if (hasConfigValue(preferredDomainsControl)) {
        epd = boolFromNotNo(preferredDomainsControl);
      }

      const preferredIPsControl = getConfigValue('epi', env.epi);
      if (hasConfigValue(preferredIPsControl)) {
        epi = boolFromNotNo(preferredIPsControl);
      }

      const githubIPsControl = getConfigValue('egi', env.egi);
      if (hasConfigValue(githubIPsControl)) {
        egi = boolFromNotNo(githubIPsControl);
      }
          ...
          if (url.pathname.length > 1 && url.pathname !== '/' && !url.pathname.includes('/sub')) {
            ...
            if (isValidFormat(user)) {
              if (user === at) {
                ...
                return new Response(
                  JSON.stringify({ error: 'UUID错误 请注意变量名称是u不是uuid' }),
                  {
                    status: 403,
                    headers: { 'Content-Type': 'application/json' },
                  }
    ...
    if (egi) {
      try {
        ...
        if (newIPList.length > 0) {
          ...
          if (et) {
            finalLinks.push(
              ...(await generateTrojanLinksFromNewIPs(newIPList, user, workerDomain))
            );
          }
        }
      } catch (error) {
...
function generateLinksFromSource(list, user, workerDomain) {
  ...
  const effectiveDomain = (hd || '').trim() || workerDomain;
...
async function fetchAndParseWetest(url) {
  try {
    ...
    const cellRegex =
      /<td data-label="线路名称">(.+?)<\/td>[\s\S]*?<td data-label="优选地址">([\d.:a-fA-F]+)<\/td>[\s\S]*?<td data-label="数据中心">(.+?)<\/td>/;
...
async function handleSubscriptionPage(request, user = null) {
  ...
  const translations = {
    zh: {
      ...
      customHomepageHint:
        '设置自定义URL作为首页伪装。访问根路径 / 时将显示该URL的内容。留空则显示默认终端页面。',
      saveConfig: '保存配置',
      advancedControl: '高级控制',
      subscriptionConverter: '订阅转换地址:',
      remoteConfigUrl: '远程配置URL (rcu):',
      remoteConfigUrlPlaceholder: '例如: https://raw.githubusercontent.com/.../tist.ini',
      remoteConfigUrlHint: '用于订阅转换时的 config 参数。留空则使用默认地址。',
      disguiseDomain: '伪装域名 (hd):',
      disguiseDomainPlaceholder: '例如: example.com',
      disguiseDomainHint: '用于节点链接的 Host/SNI。留空则使用当前 Worker 域名。',
      ...
      autoSubscriptionCopied:
        '自动识别订阅链接已复制，客户端访问时会根据User-Agent自动识别并返回对应格式',
      trojanPasswordPlaceholder: '留空则自动使用 UUID',
      trojanPasswordHint:
        '设置自定义 Trojan 密码。留空则使用 UUID。客户端会自动对密码进行 SHA224 哈希。',
      protocolHint:
        '可以同时启用多个协议。订阅将生成选中协议的节点。<br>• VLESS WS: 基于 WebSocket 的标准协议<br>• Trojan: 使用 SHA224 密码认证<br>• xhttp: 基于 HTTP POST 的伪装协议（需要绑定自定义域名并开启 gRPC）',
      saveProtocol: '保存协议配置',
                ...
                <div id="configContent" style="display: none;">
                    ...
                    <form id="otherConfigForm" style="margin-bottom: 20px;">
                        ...
                        <div style="margin-bottom: 15px;">
                          ...
                          <input type="text" id="yx" placeholder="例如: 1.2.3.4:443#日本节点,5.6.7.8:80#美国节点,example.com:8443#新加坡节点" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00ff00; color: #00ff00; font-family: 'Courier New', monospace; font-size: 14px;">
                        ...
                        <div style="margin-bottom: 15px;">
                          ...
                          <input type="text" id="yxURL" placeholder="默认: https://raw.githubusercontent.com/qwer-search/bestip/refs/heads/main/kejilandbestip.txt" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00ff00; color: #00ff00; font-family: 'Courier New', monospace; font-size: 14px;">
                        ...
                        <div style="margin-bottom: 20px; padding: 15px; background: rgba(0, 40, 0, 0.6); border: 2px solid #00aa00; border-radius: 8px;">
                            ...
                            <div style="display: flex; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; align-items: center;">
                                ...
                                <div style="width: 80px;">
                                  <label style="display: block; margin-bottom: 5px; color: #00ff00; font-size: 0.9rem;">线程</label>
                            ...
                            <div id="latencyTestResults" style="max-height: 250px; overflow-y: auto; display: none;">
                                ...
                                <div style="margin-top: 10px; display: flex; gap: 10px;">
                                  <button type="button" id="overwriteSelectedToYx" style="flex: 1; background: rgba(0, 200, 0, 0.3); border: 1px solid #00ff00; padding: 10px 20px; color: #00ff00; font-family: 'Courier New', monospace; font-weight: bold; cursor: pointer; transition: all 0.3s;">覆盖添加</button>
                                  <button type="button" id="appendSelectedToYx" style="flex: 1; background: rgba(0, 150, 0, 0.3); border: 1px solid #00aa00; padding: 10px 20px; color: #00aa00; font-family: 'Courier New', monospace; font-weight: bold; cursor: pointer; transition: all 0.3s;">追加添加</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <form id="advancedConfigForm" style="margin-bottom: 20px;">
                        ...
                        <div style="margin-bottom: 15px;">
                          ...
                          <input type="text" id="rcu" placeholder="${t.remoteConfigUrlPlaceholder}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00ff00; color: #00ff00; font-family: 'Courier New', monospace; font-size: 14px;">
                          <small style="color: #00aa00; font-size: 0.85rem;">${t.remoteConfigUrlHint}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
                          <label style="display: block; margin-bottom: 8px; color: #00ff00; font-weight: bold; text-shadow: 0 0 3px #00ff00;">${t.disguiseDomain}</label>
                          <input type="text" id="hd" placeholder="${t.disguiseDomainPlaceholder}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00ff00; color: #00ff00; font-family: 'Courier New', monospace; font-size: 14px;">
                          <small style="color: #00aa00; font-size: 0.85rem;">${t.disguiseDomainHint}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
        ...
        <script>
            ...
            async function checkSystemStatus() {
                try {
                    ...
                    document.getElementById('geoInfo').innerHTML = '检测方式: <span style="color: #ff4444;">❌ 检测失败</span>';
            ...
            async function loadCurrentConfig() {
                ...
                try {
                    ...
                    if (document.getElementById('rcu')) document.getElementById('rcu').value = config.rcu || '';
                    if (document.getElementById('hd')) document.getElementById('hd').value = config.hd || '';
                    ...
                    try {
                        const response = await fetch(window.location.pathname + '/api/config', {
                            ...
                            body: JSON.stringify({ 
                                ...
                                qj: '',
                                dkby: '',
                                yxby: '', ev: '', et: '', ex: '', tp: '', scu: '', rcu: '', epd: '', epi: '', egi: '',
                                hd: '',
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (advancedConfigForm) {
                    advancedConfigForm.addEventListener('submit', async function(e) {
                        e.preventDefault();
                    const configData = { scu: document.getElementById('scu').value, rcu: document.getElementById('rcu') ? document.getElementById('rcu').value : '', hd: document.getElementById('hd') ? document.getElementById('hd').value : '', epd: document.getElementById('epd').checked ? 'yes' : 'no', epi: document.getElementById('epi').checked ? 'yes' : 'no', egi: document.getElementById('egi').checked ? 'yes' : 'no', ae: document.getElementById('apiEnabled').value,
                    ...
                    fetchIPBtn.addEventListener('click', async function() {
                        ...
                        try {
                            ...
                            if (allItems.length > 0) {
                                ...
                                manualInputDiv.style.display = 'block';
                              showStatus('已获取 ' + allItems.length + ' 个IP', 'success');
                            } else {
                ...
                if (overwriteSelectedBtn) {
                    overwriteSelectedBtn.addEventListener('click', async function() {
                        ...
                        overwriteSelectedBtn.textContent = '保存中...';
                        ...
                        try {
                            ...
                            await saveConfig(configData);
                            showStatus('已覆盖 ' + selectedItems.length + ' 项并已保存', 'success');
                        } catch (err) {
                            showStatus('保存失败: ' + err.message, 'error');
                        } finally {
                            overwriteSelectedBtn.disabled = false;
                            appendSelectedBtn.disabled = false;
                            overwriteSelectedBtn.textContent = '覆盖添加';
                        }
                    });
                }
                ...
                if (appendSelectedBtn) {
                    appendSelectedBtn.addEventListener('click', async function() {
                        ...
                        overwriteSelectedBtn.disabled = true;
                        appendSelectedBtn.disabled = true;
                        appendSelectedBtn.textContent = '保存中...';
                        ...
                        try {
                            ...
                            await saveConfig(configData);
                            showStatus('已追加 ' + selectedItems.length + ' 项并已保存', 'success');
                        } catch (err) {
                            showStatus('保存失败: ' + err.message, 'error');
                        } finally {
                            overwriteSelectedBtn.disabled = false;
                            appendSelectedBtn.disabled = false;
                            appendSelectedBtn.textContent = '追加添加';
                        }
                    });
                }
...
function generateLinksFromNewIPs(list, user, workerDomain) {
  ...
  list.forEach((item) => {
    ...
    if (CF_HTTPS_PORTS.includes(port)) {
      ...
      const link = `${proto}://${user}@${item.ip}:${port}?encryption=none&security=tls&sni=${effectiveDomain}&fp=chrome&type=ws&host=${effectiveDomain}&path=${wsPath}#${encodeURIComponent(wsNodeName)}`;
      links.push(link);
    } else if (CF_HTTP_PORTS.includes(port)) {
      if (!disableNonTLS) {
        const wsNodeName = `${nodeName}-${port}-WS`;
        const link = `${proto}://${user}@${item.ip}:${port}?encryption=none&security=none&type=ws&host=${effectiveDomain}&path=${wsPath}#${encodeURIComponent(wsNodeName)}`;
        links.push(link);
      }
    } else {
      const wsNodeName = `${nodeName}-${port}-WS-TLS`;
...
async function generateTrojanLinksFromNewIPs(list, user, workerDomain) {
  ...
  list.forEach((item) => {
    ...
    if (CF_HTTPS_PORTS.includes(port)) {
      ...
      const link = `${atob('dHJvamFuOi8v')}${password}@${item.ip}:${port}?security=tls&sni=${effectiveDomain}&fp=chrome&type=ws&host=${effectiveDomain}&path=${wsPath}#${encodeURIComponent(wsNodeName)}`;
      links.push(link);
    } else if (CF_HTTP_PORTS.includes(port)) {
      if (!disableNonTLS) {
        const wsNodeName = `${nodeName}-${port}-${atob('VHJvamFu')}-WS`;
        const link = `${atob('dHJvamFuOi8v')}${password}@${item.ip}:${port}?security=none&type=ws&host=${effectiveDomain}&path=${wsPath}#${encodeURIComponent(wsNodeName)}`;
        links.push(link);
      }
    } else {
      const wsNodeName = `${nodeName}-${port}-${atob('VHJvamFu')}-WS-TLS`;
  ...
  if (!ae) {
    return new Response(
      JSON.stringify({
        ...
        error: 'API功能未启用',
        message:
          '出于安全考虑，优选IP API功能默认关闭。请在配置管理页面开启"允许API管理"选项后使用。',
      }),
      {
        status: 403,
        headers: { 'Content-Type': 'application/json' },
...
function updateConfigVariables() {
  ...
  remoteConfigUrl =
    getConfigValue('rcu', '') || getConfigValue('remoteConfigUrl', '') || DEFAULT_REMOTE_CONFIG_URL;

  hd = getConfigValue('hd', '') || '';
